...
void decrypt() {
    ...
    printf("Selected mode: Decryption\nPlease enter Base64 string to decrypt: ");
    ...
    for (int i = 0; i < len; i++) {
        ...
        int decimal = base64ToDecimal(input_string[i]);

        // Convert to binary
        for (int j = 5; j >= 0; j--) {
            binary_string[bin_index++] = ((decimal >> j) & 1) + '0';
        }
    }

    binary_string[bin_index] = '\0';

    // STEP 2: Break binary string into 8-bit chunks, convert to ASCII
    int str_index = 0;
    for (int i = 0; i + 7 < bin_index; i += 8) {
        char byte[9];
        strncpy(byte, &binary_string[i], 8);
        byte[8] = '\0';
        output[str_index++] = (char)convertToDecimal(byte);
    }
